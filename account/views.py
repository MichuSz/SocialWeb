...
def edit(request):
    if request.method == "POST":
        ...
        if user_form.is_valid() and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            messages.success(request,"Uaktualnianie profilu zakonczylo sie sukcesem.")
        else:
            messages.error(request,"Wystapil blad podczas uaktualniania profilu.")
    else:
...
def user_detail(request,username):
    ...
    return render(request,'account/user/detail.html',{'section':'poeple','user':user})
